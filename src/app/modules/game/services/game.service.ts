...
export class GameService {
  ...
  private size = 4
  private availbCells: number[] = []

  private get emptyCells(): number[] {
    const notEmptyCells = this.notEmptyCells
     
    return this.availbCells.filter(position => !notEmptyCells.includes(position))
  }
  ...
  private move(dimX: 'col' | 'row' = 'row', dimY: 'col' | 'row' = 'col', reverse: boolean = false ) {
    ...
    for(let x = 1; x <= this.size; x++){
      const rowItems = this.items.filter(item => item[dimX] === x).sort((a, b) => a[dimY] - b[dimY])
      if(reverse) {
        rowItems.reverse()
      }
    ...
    this.scores += mergedItems.reduce((acc:number, item: Item) => acc + item.value, 0)
  ...
  private canIMove(dimX: 'row' | 'col', skipDir: boolean = true, forward: boolean = false) {
    for (let x = 1; x <= this.size; x++) {
      const dimY = dimX === 'row' ? 'col' : 'row'
      const items = this.items.filter(item => !item.isOnDelete && item[dimX] === x).sort((a, b) => a[dimY] - b[dimY])
  ...
  private generateAvailableCells() {
    for (let row = 1; row <= this.size; row++) {
      for (let col = 1; col <= this.size; col++) {
        this.availbCells.push(row * 10 + col)
      }
    }
  }
}
